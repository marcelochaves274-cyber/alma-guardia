...
export function ViewPe() {
    ...
    const fetchContent = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          setContent(data.peContent || 'Seu texto aqui');
        } else {
          setContent('Seu texto aqui');
        }
      } catch (error) {
        console.error("Error fetching PE content:", error);
        toast({
          variant: "destructive",
          title: "Erro ao carregar",
          description: "Não foi possível buscar o Plano de Emergência.",
        });
      } finally {
        setIsLoading(false);
      }
  ...
  const handleSaveContent = async () => {
    ...
    setIsSaving(true);
    try {
      await setDoc(docRef, { peContent: content }, { merge: true });
      toast({
        title: 'Sucesso!',
        description: `Conteúdo do PE foi salvo.`,
      });
      setIsEditing(false);
    } catch (error) {
      console.error(`Error saving PE content:`, error);
      toast({
        variant: 'destructive',
        title: 'Erro ao salvar',
        description: `Não foi possível salvar o conteúdo do PE.`,
      });
    } finally {
      setIsSaving(false);
    }
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardTitle>PE - Plano de Emergência</CardTitle>
        <CardDescription>Visualize ou edite o conteúdo do seu Plano de Emergência.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <Textarea value={content} onChange={(e) => setContent(e.target.value)} placeholder={isLoading ? "Carregando..." : "Digite o conteúdo do PE aqui..."} className="min-h-[400px] text-base" readOnly={!isEditing} disabled={isSaving || isLoading} />
      </CardContent>
      {profile === 'admin' && (
