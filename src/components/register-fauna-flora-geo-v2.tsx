...
export function RegisterFaunaFloraGeoV2({ recordToEdit, setPage, prefillData }: RegisterFaunaFloraGeoV2Props) {
  const isEditing = !!recordToEdit;

  // Form states based on register-occurrence logic
  ...
  const [locationName, setLocationName] = useState(recordToEdit?.locationName || prefillData?.location || '');
  ...
  // Populate form if we are editing (from register-occurrence)
  useEffect(() => {
    if (isEditing && recordToEdit) {
      ...
      // Correct location handling for editing
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const recordData = {
        ...
        description,
        analysis,
        location, // Correct object structure
        userId: user.uid,
    };
    ...
    try {
      if (isEditing && recordToEdit) {
        ...
        setPage('fauna-flora-geo-report-v2');
      } else {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <CardFooter className="flex flex-col gap-2">
          <Button type="submit" className="w-full" disabled={isSubmitting}>
            ...
            {isEditing && (<Button variant="outline" type="button" className="w-full" onClick={() => setPage('fauna-flora-geo-report-v2')}>Cancelar Edição</Button>)}
        </CardFooter>
      </form>
